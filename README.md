...
- **Engine, target:** portable **ONNX** core (Rust or equivalent) running the same model family — one headless engine that later embeds in an Obsidian plugin sidecar, a Mac app, and eventually on-device mobile.
...
Docs-first. Milestone 1 (PDF → vault-quality Markdown on Mac) in progress — see [PLAN.md](./PLAN.md).
