...
| 2026-07-28 | M4 | — | **0.1.0 failed review; 0.1.1 answers it.** Two blocking Errors, one cause: inline `eslint-disable` comments for `obsidianmd/*` rules are rejected outright — the console logging they suppressed was only a Warning. Directives removed and the logging with them (the two dev-only `console.log`s became `backendSpoof()` / `lastWorkerEnv()` on the probe, so the driver can assert on them); the worker's warn/error bridge is relaxed in `eslint.config.mjs` where it is visible instead. **The finding worth keeping: the review reads the bundle, not the intent.** `main.js` carried `require("fs")`/`path`/`os` and the plugin was described to users as able to "read and write any file on the system" — all of it inside the onnxruntime emscripten glue, inlined *as a text string* and, since ORT fixed the renderer guard upstream ~1.24, never used at all. 92 KB of dead text bought a filesystem warning on a public scorecard. `analyzeOrtGlue()` now decides at build time and inlines only when a patch is needed (the loud failure on an unrecognised epilogue stays, so a future ORT bump still can't reintroduce the bug quietly); the only `require()` left in the bundle is `require("obsidian")`, and `main.js` drops to 4582566 B. Encoding the string to hide it would also have "worked" — and is obfuscation, which the developer policies prohibit. Also: pdf.js `isEvalSupported: false` on both paths so a document's Type-4 shading functions are interpreted, not compiled — priced first against the ground-truth suite (attention **8/8**, 376 s, identical to baseline); `authorUrl` → org profile; release assets now carry SLSA provenance attestations (`gh attestation verify` ✓ at `refs/tags/0.1.1`). Released 0.1.1 artifacts re-verified end-to-end through the real command path in a clean vault. |
| 2026-07-28 | — | — | **Bookkeeping, recorded because the history is misleading.** Commit `6d802a8`, whose message is entirely about the release pre-release flag, also contains ~1300 lines of the EPUB spike that was uncommitted in the tree at the time — `engine-js/tools/md2epub.mjs`, the formula bbox/LaTeX work in `doctags.ts` and its two new tests, `meta.ts`, `plugin/tools/mathjax-probe.js`, and a `plugin/main.ts` change. Swept in by a `git add -A` that nobody inspected. Left as-is deliberately: rewriting that commit would change the SHA of every descendant including `030ae89`, which is what the `0.1.1` tag, the build-provenance attestation signed over `refs/tags/0.1.1`, and the pending directory review all point at. A misleading commit message is the cheaper of the two problems. |
